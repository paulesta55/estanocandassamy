...
\item Si un \emph{STATE\_EVENT} (correspondant changement d'état lié à l'environnement ou à la partie) la méthode \emph{updateMap()} est appelée dans le cas où le joueur change de niveau (évènement \emph{LEVEL\_CHANGE}, les méthode \emph{updateMap()} et \emph{updatePlayers()} sont toutes deux appelées si l'état subit un changement global.
\end{itemize}
