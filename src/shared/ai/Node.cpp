...
std::vector<ai::Node*> ai::Node::getAvailableNeigbors (uint authorizedTiles,state::Map* m){
    state::Position current = this->position;
    std::vector <ai::Node*> list_Node;
    ...
    for (auto neighbor : positionNeigbors){
        if(m->getLayers()->at(0).getData()->at(static_cast<unsigned long>(neighbor)) == authorizedTiles){
            list_Node.push_back(new Node(this, state::Position(neighbor%m->getWidth(),
